...
# awk '{print $10" "$11;}' ../$GROUP.photos | sort -k1 -k2 > $GROUP.geo
# 
...
# cat $GROUP.geo | awk '{printf("https://maps.googleapis.com/maps/api/streetview?size=100x100&location=%s,%s&heading=0&pitch=0&key=%s\n", $1, $2, "AIzaSyB6I_6gNeXnleE_ojsCSh-a9mFSrVwMsc0");}' > streetview.url
# cat $GROUP.geo | awk '{printf("https://maps.googleapis.com/maps/api/streetview?size=100x100&location=%s,%s&heading=90&pitch=0&key=%s\n", $1, $2, "AIzaSyB6I_6gNeXnleE_ojsCSh-a9mFSrVwMsc0");}' >> streetview.url
# cat $GROUP.geo | awk '{printf("https://maps.googleapis.com/maps/api/streetview?size=100x100&location=%s,%s&heading=180&pitch=0&key=%s\n", $1, $2, "AIzaSyB6I_6gNeXnleE_ojsCSh-a9mFSrVwMsc0");}' >> streetview.url
# cat $GROUP.geo | awk '{printf("https://maps.googleapis.com/maps/api/streetview?size=100x100&location=%s,%s&heading=270&pitch=0&key=%s\n", $1, $2, "AIzaSyB6I_6gNeXnleE_ojsCSh-a9mFSrVwMsc0");}' >> streetview.url
...
# # 
# # lenstr=$(wc -l $GROUP.streetview_url)
# # len=$(echo $lenstr | cut -d' ' -f1)
# # echo $len
# # 
# # head -n $len $FILENAME's' > $FILENAME'_sed'
# # 
# # paste -d ' ' $FILENAME'_sed' $GROUP.streetview_url > streetview.urls
# # 
# # rm $FILENAME*
