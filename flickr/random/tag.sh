...
for i in {1..65}; do
  ...
  ${FLICKR_DIR}/scripts/join_lookup.sh 1 1 tag/femaleuidsort$i tag/$GROUP.photosuidsort | awk '{print $1" "$20;}' | uniq > tag/femaleuidtag$i
...
# ${FLICKR_DIR}/scripts/join_lookup.sh 1 1 tag/femaleuidsort tag/$GROUP.photosuidsort | awk '{print $1" "$20;}' | uniq > tag/femaleuidtag
...
wc -l tag/femaleuidsort* | sed 's/femaleuidsort//g' | sort -g -k2 | awk '{print $1;}' | sed '1d' > tag/allpeople.femalecountuidperbin
